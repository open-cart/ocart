...
{{--        <img src="{{ get_object_image($value, Arr::get($attributes, 'allow_thumb', true) == true ? 'thumb' : null) }}" alt="{{ __('preview image') }}" class="preview_image" @if (Arr::get($attributes, 'allow_thumb', true)) width="150" @endif>--}}
        <img src="{{ TnMedia::getImageUrl($value, 'thumb', '/images/placeholder.png') }}" alt="{{ __('preview image') }}" class="preview_image h-32" @if (Arr::get($attributes, 'allow_thumb', true)) width="150" @endif>
...
<script>
    if (typeof removeImageHandler === 'undefined') {
        ...
        $(function() {
            const img = $('.preview-image-wrapper').first().clone();
                ...
                TnMedia.default({
                    ...
                    insert: (items) => {
                        ...
                        if (!parent.find('.preview-image-wrapper img').length) {
                            ...
                            newImg.find('img').attr('src', items[0].thumb);
                            parent.prepend(newImg);
                        } else {
                            parent.find('.preview_image').attr('src', items[0].thumb);
                        }
                    }
                })
            })
