...
class MetaBox
{
    ...
    public function __construct(MetaBoxRepository $metaBoxRepository)
    {
    ...
    {
        try {
            $fieldMeta = $this->metaBoxRepository->findWhere([
                'meta_key'       => $key,
    ...
    {
        return $this->metaBoxRepository->findWhere([
            'meta_key'       => $key,
            'reference_id'   => $object->id,
            'reference_type' => get_class($object),
        ], $select)->first();
    ...
    {
        return $this->metaBoxRepository->deleteWhere([
            'meta_key'       => $key,
            'reference_id'   => $object->id,
            'reference_type' => get_class($object),
        ]);
