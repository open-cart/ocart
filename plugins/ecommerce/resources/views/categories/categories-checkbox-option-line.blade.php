            ...
            @if($category->id != $currentId)
                <li value="{{ $category->id ?? '' }}"
                    ...
                    <label class="flex items-center space-x-3">
                        {!! Form::checkbox($name, $category->id, in_array($category->id, $value), ['class' => 'flex-none rounded-md h-4 w-4 border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50']) !!}
